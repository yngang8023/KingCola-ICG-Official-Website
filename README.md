...
- 运行方式: 静态前端站点（当前部署目标为 EdgeOne Pages，也可适配 Nginx 等平台）
...
## 8.1 EdgeOne Pages 部署

当前站点可直接部署到 `EdgeOne Pages`，适合静态站点托管、全球 CDN 加速与边缘分发场景。
...
项目仍保留 `vercel.json`，已配置 SPA rewrite 到 `/`，如需迁移也可继续使用。

## 8.4 GitHub Actions 自动部署到云服务器
